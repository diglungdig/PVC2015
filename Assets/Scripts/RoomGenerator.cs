...
public class RoomGenerator : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			//Generate floor
			StartCoroutine(WaitSecs(newRoom));
	...
	IEnumerator WaitSecs(Room newRoom){
		foreach (Tile tile in newRoom.floorTiles) {

			GameObject temp = (GameObject)Instantiate (floorTile, new Vector3 (tile.x, -3, tile.z), Quaternion.identity);
			temp.transform.parent = room.transform;
			//temp.transform.Translate(new Vector3(0,0,5.0f)*Time.deltaTime, Space.World);
			roomTiles.Add (temp);

			Vector3 source = temp.transform.position;
			float startTime = Time.time;
			while(Time.time < startTime + 0.5f){
				temp.transform.position = Vector3.Lerp(source, new Vector3 (tile.x, 0, tile.z), (Time.time - startTime)/0.5f);
				yield return null;

			}
			temp.transform.position = new Vector3 (tile.x, 0, tile.z);
	...
	public Room CrossGenerator(Room room){
		...
		Vector3 centerPoint1 = room.centerPoint;
		Vector3 centerPoint2 = new Vector3 (room.centerPoint.x, room.centerPoint.y, room.centerPoint.z + (width2 / 2 + room.widthWithoutBound / 2 ));
		Vector3 centerPoint3 = new Vector3 (room.centerPoint.x, room.centerPoint.y, room.centerPoint.z - (width2 / 2 + room.widthWithoutBound / 2));
		...
		//!lowerOne
		int halfLengthLeft3 = (int)(centerPoint3.x -  length2 / 2) + 1;
		int halfLengthRight3 = (int)(centerPoint3.x + length2 / 2) - 1;
		int halfWidthBot3 = (int)(centerPoint3.z - width2 / 2) + 1;
		int halfWidthTop3 = (int)(centerPoint3.z + width2 / 2) - 1;
		
		for (int x = halfLengthLeft3; x <= halfLengthRight3 ; x = x+2) {
			for (int z = halfWidthBot3; z <= halfWidthTop3; z = z+2) {
				Debug.Log(z);
				Debug.Log(x);
				room.floorTiles.Add(new Tile(1,x,z,0));
			}
