...
#region PointerEventData pool
        ...
        protected MouseState GetGazePointerData()
        {
            ...
            if (physicsRaycaster)
            {
                ...
                //sending raycasthit object to grab&drop script

                //enable highlighting
                if (leftData.pointerCurrentRaycast.gameObject.GetComponent<HighlightableObject>() == null && leftData.pointerCurrentRaycast.gameObject.tag == "grabable")
                {
                    leftData.pointerCurrentRaycast.gameObject.AddComponent<HighlightableObject>();
                }
                else if(leftData.pointerCurrentRaycast.gameObject.tag == "grabable")
                {
                    if (leftData.pointerCurrentRaycast.gameObject.GetComponent<HighlightableObject>().enabled == true)
                    {
                        leftData.pointerCurrentRaycast.gameObject.GetComponent<HighlightableObject>().On();
                    }  
                }
                //need to figure out a way to get rid of hightlightableobject when not looking at them

                if (Input.GetMouseButtonDown(0) || OVRInput.Get(OVRInput.Button.One)) {
					grabdrop.TryGrabObject (leftData.pointerCurrentRaycast.gameObject);
				}
